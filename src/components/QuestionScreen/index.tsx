...
import { AppLogo, Back, CheckIcon, Next, TimerIcon } from '../../config/icons'
...
const QuestionScreen: FC = () => {
  ...
  const { question, type, choices, code, image, audio, correctAnswers } = currentQuestion
  ...
  const onClickBack = () => {
    ...
    if (prevQuestionIndex >= 0) {
      ...
      // Handle the case when there are no more previous questions
      console.log('No more previous questions')
    }
  }

  const onClickSkip = () => {
    // Add your skip logic here
    // For example, you might want to move to the next question without selecting an answer
    const nextQuestionIndex = activeQuestion + 1
    if (nextQuestionIndex < questions.length) {
      setActiveQuestion(nextQuestionIndex)
      // You might want to clear any selected answer state
      // setSelectedAnswer([]);
    } else {
      // Handle the case when all questions are skipped
      // e.g., navigate to a different page or finish the quiz
      console.log('All questions skipped')
    }
  }
  ...
  return (
    <PageCenter>
      ...
      <QuizContainer selectedAnswer={selectedAnswer.length > 0}>
        ...
        <ButtonWrapper>
          <Button
            text={activeQuestion === questions.length - 1 ? 'Back' : 'Cancel'}
            onClick={onClickBack}
            icon={<Back />}
            iconPosition="left"
